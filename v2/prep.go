...
func (route *Route) prepDomainAndSubdomain() routePrepError {
	...
	if route.Domain == nil {
		...
		if route.Domain == nil {
			...
			route.Subdomain = &Subdomain{ Name: "", online: true, }
...
func prepDomainAndSubdomainLocal(route *Route) {
	...
	route.Srv.offlineClientsM.RLock()
	savedConfig, ok := route.Srv.offlineClients[route.RemoteAddress]
	route.Srv.offlineClientsM.RUnlock()
