...
func (route *Route) ServePHP(php *PHPProcessor, dir string) {
	h := gofast.NewHandler(
		gofast.NewFileEndpoint(dir)(gofast.BasicSession),
		gofast.SimpleClientFactory(php.connFactory),
	)

	phpLogger := route.Logger.Clone(nil, true, "php")

	h.SetLogger(log.New(phpLogger, "PHP", 0))
	h.ServeHTTP(route, route.R)
}
