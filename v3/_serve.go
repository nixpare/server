...
func (route *Route) serveError() {
	route.disableErrorCapture = true

	if len(route.caputedError) != 0 {
		if strings.Contains(http.DetectContentType(route.caputedError), "text/html") {
			route.W.Write(route.caputedError)
		} else {
			route.errMessage = string(route.caputedError)
		}
	}
...
func (route *Route) ServeFile(filePath string) bool {
	...
	if value, ok := route.Website.XFiles[route.R.URL.Path]; ok {
...
func (route *Route) serveXFile(xFilePath string) bool {
	...
	route.ServeCompressedContent(route.R.URL.Path, modTime, bytes.NewReader(content), gzip.DefaultCompression)
	return true
}
...
func (route *Route) StaticServe(serveHTML bool) bool {
	...
	for _, s := range route.Website.HiddenFolders {
		if s == "" || strings.HasPrefix(route.R.URL.Path, s) {
			return route.Error(http.StatusNotFound, "Not Found")
		}
	}

	if route.R.URL.Path == "/" && serveHTML {
		return route.ServeFile(route.Website.Dir + "/index.html")
	}

	if strings.HasSuffix(route.R.URL.Path, ".html") && !serveHTML {
		return route.Error(http.StatusNotFound, "Not Found")
	}

	for _, s := range route.Website.AllFolders {
		if s == "" || strings.HasPrefix(route.R.URL.Path, s) {
			return route.serveFileTestHTML(route.Website.Dir + route.R.URL.Path)
		}
	}
...
func (route *Route) SetCookie(name string, value any, maxAge int, secure bool) error {
...
func (route *Route) SetCookiePerm(name string, value any, maxAge int, secure bool) error {
...
func (route *Route) NewReverseProxy(dest string) (*httputil.ReverseProxy, error) {
	...
	reverseProxy := &httputil.ReverseProxy {
		Rewrite: func(pr *httputil.ProxyRequest) {
			...
			pr.Out.RequestURI = route.R.URL.Path
			
			var query string
			if queryMap := route.R.URL.Query(); len(queryMap) != 0 {
				first := true
...
func (route *Route) IsInternalConn() bool {
	if strings.Contains(route.R.RemoteAddr, "localhost") || strings.Contains(route.R.RemoteAddr, "127.0.0.1") || strings.Contains(route.R.RemoteAddr, "::1") {
		return true
	}

	return route.Srv.IsInternalConn(route.R.RemoteAddr)
}

func (route *Route) IsLocalhost() bool {
	if strings.Contains(route.R.Host, "localhost") || strings.Contains(route.R.Host, "127.0.0.1") || strings.Contains(route.R.Host, "::1") {
		return true
	}

	return route.Srv.IsLocalhost(route.R.Host)
}
